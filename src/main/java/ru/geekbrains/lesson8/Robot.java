...
public class Robot implements Movable {
    ...
    public boolean run(Obstacle obstacle) {
        if (runLength < obstacle.getSize()) {
            System.out.println("Робот " +  this.name + " не смог пробежать " + obstacle);
            return false;
        } else {
            System.out.println("Робот " +  this.name + " успешно пробежал " + obstacle);
            return true;
        }
    }
    ...
    public boolean jump(Obstacle obstacle) {
        if (jumpHeight < obstacle.getSize()) {
            System.out.println("Робот " +  this.name + " не смог перепрыгнуть " + obstacle);
            return false;
        } else {
            System.out.println("Робот " +  this.name + " успешно перепрыгнул " + obstacle);
            return true;
        }
    }
    ...
    public void pass(Obstacle obstacle) {
        if (inRace && !obstacle.canPass(this)) {
            ...
            System.out.println("Робот " + name + " выбыл!");
        }
    }
}
